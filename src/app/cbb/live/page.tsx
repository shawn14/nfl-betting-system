...
export default function CBBLiveTrackerPage() {
  ...
  useEffect(() => {
    const loadPredictions = async () => {
      try {
        const response = await fetch('/cbb-prediction-data.json', { cache: 'no-cache' });
        const data = await response.json();
        if (data?.games) {
    ...
    <RequireAuth>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-4">
        <div className="flex items-center justify-between border-b border-gray-200 pb-3">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold text-gray-900">CBB Live Pace Tracker</h1>
