...
export default function WNBARankingsPage() {
  ...
  useEffect(() => {
    const fetchTeams = async () => {
      try {
        const res = await fetch('/wnba-prediction-data.json', { cache: 'no-cache' });
  ...
  // Define tiers for WNBA (30 teams)
  const tiers = [
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-gray-200 pb-3">
        <div className="flex items-center gap-3">
          <h1 className="text-xl font-bold text-gray-900">WNBA Power Rankings</h1>
          <span className="bg-orange-100 text-orange-700 text-xs font-bold px-2 py-1 rounded">WNBA</span>
        </div>
                    ...
                    return (
                      <tr key={team.id} className="hover:bg-gray-50 transition-colors cursor-pointer">
                        <td className="px-4 py-3">
                          <Link href={`/wnba/teams/${team.abbreviation.toLowerCase()}`} className="block">
                        ...
                        <td className="px-4 py-3">
                          <Link href={`/wnba/teams/${team.abbreviation.toLowerCase()}`} className="flex items-center gap-3">
