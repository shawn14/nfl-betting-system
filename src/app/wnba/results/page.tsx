...
function computeAnalysis(results: BacktestResult[]): Analysis {
  ...
  for (const r of results) {
    ...
    // WNBA spread sizes (different thresholds than NFL)
...
export default function WNBAResultsPage() {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      try {
        const blobRes = await fetch('/wnba-prediction-data.json', { cache: 'no-cache' });
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border-b border-gray-200 pb-4">
        <div className="flex items-center gap-3">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">WNBA Results</h1>
