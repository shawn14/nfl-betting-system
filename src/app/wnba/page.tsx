...
export default function WNBADashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const response = await fetch('/wnba-prediction-data.json');
      const data = await response.json();
  ...
  const syncAll = useCallback(async () => {
    setSyncing(true);
    try {
      await fetch('/api/cron/wnba-sync');
      await fetchData();
    } catch (err) {
  ...
  if (loading || syncing) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh]">
        ...
        <p className="text-gray-500 text-sm">{syncing ? 'Syncing WNBA data...' : 'Loading...'}</p>
      </div>
    );
  }
  ...
  return (
    <RequireAuth>
      ...
      <div className="flex flex-col gap-3 border-b border-gray-200 pb-3 sm:flex-row sm:items-center sm:justify-between">
        <h1 className="text-xl font-bold text-gray-900">WNBA Picks</h1>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {[...displayGames]
            ...
            .map(({ game, prediction }) => {
              ...
              const pickOver = hasVegas ? ourTotal > prediction.vegasTotal! : ourTotal > 165;
              ...
              return (
                <div key={game.id} className="relative bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden hover:shadow-md hover:border-gray-300 transition">
                  ...
                  {/* Game header */}
                  <a href={`/wnba/game/${game.id}`} className="group block p-3 sm:p-4 border-b border-gray-100 hover:bg-gray-50 transition-colors">
      ...
      {displayGames.some(({ game }) => game.status === 'final') && (() => {
        ...
        displayGames.filter(({ game }) => game.status === 'final').forEach(({ game, prediction }) => {
          ...
          const vegasTotal = prediction.vegasTotal ?? 165;
