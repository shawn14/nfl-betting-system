...
- **Injury cache hygiene.** `injuriesByWeek` entries for 2025 weeks 16-18 are the old hardcoded list (source undefined, keyed `WAS`) and are kept only as history — do not treat them as real. On load the NFL cron purges any entry at or beyond the current week whose `data.source !== 'espn'`, and the stale-cache fallback only reuses feed-sourced data.
